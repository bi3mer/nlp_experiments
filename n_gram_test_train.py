...
min_grammar_size = 2
max_grammar_size = 10
training_data_split = 0.8 # 80% of data is in the training set
...
print('\nReading in files and splitting between test and training sets')
for file_name in tqdm(files):
    ...
    slice_index = int(len(content) * training_data_split)
    training_data.append(content[:slice_index])
    test_data.append(content[len(content) - slice_index:])

print('\nBuilding Grammars')
for training_set in tqdm(training_data):
    for word in tqdm(training_set):
...
print('\n\nFinding the most common occurring key to start a sentence')
best_key = ''
most_occurrences = -1
...
def write_to_html(f, starting_input, compiled_grammar, grammar_size):
    ...
    f.write('\n<br/>\n<br/>\n')
    f.write(f'<b>Percent Likelihood:</b> {compiled_grammar.sequence_probability(paragraph) * 100}%<br/>')

    for i in range(len(test_data)):
        f.write(f'<b>Perplexity[{i}]:<b/> {compiled_grammar.perplexity(test_data[i])}')
